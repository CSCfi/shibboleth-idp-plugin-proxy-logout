 ...
 * {@link net.shibboleth.idp.authn.AuthenticationResult} based on an inbound
 * SAML 2.0 SSO response.
 * 
 * <p>
 * A {@link SAMLAuthnContext} is used as the basis of the result and the lack of
 * a context is a signal to record a failure. Actual validation is all upstream
 * of this action, but the use of the ValidationAction subclass is a convenience
 * for auditing and handling the result.
 * </p>
 * 
 ...
 *      </pre>
 * 
 * @post If AuthenticationContext.getSubcontext(SAMLAuthnContext.class) != null,
 *       then an {@link net.shibboleth.idp.authn.AuthenticationResult} is saved
 *       to the {@link AuthenticationContext}.
 */
public class ValidateSAMLAuthentication extends AbstractValidationAction {
    ...
    /**
     * Strategy used to look up a {@link RelyingPartyContext} for configuration
     * options.
     */
    @Nonnull
    ...
    /**
     * An IdPAttribute ID to log as a "name" in place of the NameID for "info"
     * purposes.
     */
    @Nullable
    @NotEmpty
    ...
    /**
     * Set the strategy used to return the {@link RelyingPartyContext} for
    ...
    /**
     * Log a successful authentication based on a designated attribute ID or the
     * NameID value.
     */
...
// Checkstyle: CyclomaticComplexity|MethodLength OFF
    ...
    protected Subject populateSubject(@Nonnull final Subject subject) {
        ...
        if (authnContextTranslatorEx != null) {
            ...
            final Collection<Principal> translated = authnContextTranslatorEx
                    .apply(localSamlAuthnContextParent.getSubcontext(ProfileRequestContext.class));
...
// Checkstyle: CyclomaticComplexity|MethodLength ON
    ...
    /**
     * Construct a populated {@link ProxyAuthenticationPrincipal} based on the
     * inbound assertion.
     * 
     * @param authnContext the SAML {@link AuthnContext} issued by the proxied IdP
     * 
     * @return a constructed {@link ProxyAuthenticationPrincipal} to include in the
     *         {@link Subject}
     */
    @Nonnull
    private ProxyAuthenticationPrincipal buildProxyPrincipal(@Nonnull final AuthnContext authnContext) {
        ...
        if (!authnContext.getAuthenticatingAuthorities().isEmpty()) {
            proxied.getAuthorities()
                    .addAll(authnContext.getAuthenticatingAuthorities().stream().map(AuthenticatingAuthority::getURI)
                            .filter(aa -> !Strings.isNullOrEmpty(aa)).collect(Collectors.toUnmodifiableList()));
        }
        ...
        if (condition != null) {
            ...
            if (audiences != null) {
                proxied.getAudiences().addAll(audiences.stream().map(Audience::getURI)
                        .filter(a -> !Strings.isNullOrEmpty(a)).collect(Collectors.toUnmodifiableList()));
            }
        }
    ...
    private void processAttributes(@Nonnull final ProfileRequestContext profileRequestContext) {
        ...
        try (final ServiceableComponent<AttributeTranscoderRegistry> component = transcoderRegistry
     ...
     * 
     * @throws AttributeDecodingException if an error occurs or no results were
     *                                    obtained
     */
    ...
    private void filterAttributes(@Nonnull final ProfileRequestContext profileRequestContext) {
        ...
        final AttributeFilterContext filterContext = profileRequestContext
                .ensureSubcontext(AttributeFilterContext.class);
    ...
    private void populateFilterContext(@Nonnull final ProfileRequestContext profileRequestContext,
        ...
        filterContext.setDirection(Direction.INBOUND)
                ...
                .setMetadataResolver(metadataResolver).setRequesterMetadataContextLookupStrategy(null)
                .setIssuerMetadataContextLookupStrategy(new SAMLMetadataContextLookupFunction()
                        .compose(new RecursiveTypedParentContextLookup<>(ProfileRequestContext.class)))
                .setProxiedRequesterContextLookupStrategy(null)
                .setAttributeIssuerID(Constraint.isNotNull(getResponderLookupStrategy(), "No responder Strategy")
                        .apply(profileRequestContext))
                .setAttributeRecipientID(Constraint.isNotNull(getRequesterLookupStrategy(), "No requester strategy")
                        .apply(profileRequestContext));
    }
